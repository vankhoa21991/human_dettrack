...
class Evaluator:
    ...
    def eval_setup(self, opt):
        ...
        gt_folder = data_path / self.opt.benchmark / self.opt.split
        mot_seqs_path = data_path / opt.benchmark / opt.split
        ...
        LOGGER.info(f'Sequence paths: {seq_paths}')
    ...
    def evaluate(self, seq_paths, save_dir, gt_folder):
        # run the evaluation on the generated txts
        d = [seq_path.parent.name for seq_path in seq_paths]
        p = subprocess.Popen(
            args=[
                sys.executable, 'dettrack/utils/TrackEval/scripts/run_mot_challenge.py',
                "--GT_FOLDER", gt_folder,
                "--BENCHMARK", "",
    ...
    def eval(self, opt, seq_paths, save_dir, gt_folder, free_devices):
        ...
        if not self.opt.eval_existing:
            self.track(seq_paths, save_dir, free_devices)
        ...
        stdout = self.evaluate(seq_paths, save_dir, gt_folder)
    ...
    def run(self, opt):
        ...
        """
        e = Evaluator(opt)
        seq_paths, save_dir, gt_folder = e.eval_setup(opt)
        free_devices = e.device_setup(opt, seq_paths)
        results = e.eval(opt, seq_paths, save_dir, gt_folder, free_devices)
